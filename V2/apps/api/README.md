...
**已實作完成。** 12 支端點都在 `/api/v2`——獲客 6：`health`、
...
`me/action-cards/{id}/feedback`、`events/batch`。
deterministic 計算在 `packages/mindfolio-core`；AI 敘事（Bedrock + 固定模板
fallback，絕不 raise）在 `ai/`；持股與 reports/feedback/events 走 PostgreSQL 四表
schema（DB 不可用 → 503）。V2 Python suite 60 tests 綠，並對真實 300 檔 catalog 驗證。
