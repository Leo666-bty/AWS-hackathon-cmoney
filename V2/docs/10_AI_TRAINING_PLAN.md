...
主辦方資料約有 300 檔股票、2025 年每日市場資料，可聚合成約 3,500 個 stock-month samples。
...
→ StandardScaler
→ KMeans / Gaussian Mixture candidates
→ silhouette + stability selection
...
## Target Python project boundary（尚未實作）

目前 `apps/ai-training` 只有 artifact metadata、feature contract 與
`mindfolio-training-status` CLI；`packages/mindfolio-core/features` 也是空的 package
boundary。下列檔案樹是第一版 training slice 的目標，不代表現有檔案。
...
FastAPI 與 AI training 使用同一個 Python virtual environment／lockfile，但執行方式不同：

- `apps/api`：online inference 與 HTTP requests。
...
FastAPI 啟動時載入已核准 artifact，並透過固定 service contract 使用：
...
3. 保存 artifact metadata 與評估結果。
4. FastAPI 對五檔買進月份執行 inference。
5. React Result 顯示 regime distribution、兩個顯著月份與 evidence。
