...
│   │   ├── stocks.py
│   │   ├── reconstructions.py
│   │   ├── retention.py      # 留存 6 端點（auth/session、reports、me、events）
...
│       ├── market_data.py
│       ├── holdings.py
│       └── retention.py      # reports / feedback / events 持久層
...
留存側 6 個端點同樣在 `/api/v2` 底下，身份一律由 session token 推導，**不接受**
client 傳入的 `user_id`。
...
**React × FastAPI 兩階段 vertical slice 已完成**（12 支端點：獲客 6 — health、stocks
popular/search/envelope、reconstructions validate/complete；留存 6 — auth/session、
reports claim、reports confirmed-holdings、me/dashboard、me/action-cards feedback、
events/batch；V2 Python suite 60 tests 與 React production build 通過）。市場資料走檔案
catalog、確認持股與 reports/feedback/events 走 Postgres 四表 schema。`V2/demo/` 是靜態互動
reference；`apps/ai-training/` 仍是離線模型 scaffold（狀態 stub，尚未訓練）。
